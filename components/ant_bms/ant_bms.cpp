...
void AntBms::on_ant_bms_data_(const uint8_t &function, const std::vector<uint8_t> &data) {
  if (data[0] == 0xAA && data.size() == 140) {
    this->on_status_data_(data);
    return;
  }

  if (data[0] == 0x7E) {
    this->on_v2021_status_data_(data);
    return;
  }
...
void AntBms::on_v2021_status_data_(const std::vector<uint8_t> &data) {
  auto ant_get_16bit = [&](size_t i) -> uint16_t {
  ...
  ESP_LOGI(TAG, "  Number of cells: %d", cells);
  ...
  for (uint8_t i = 0; i < cells; i++) {
    ESP_LOGI(TAG, "  Cell voltage %d: %.3f V", i + 1, ant_get_16bit(i * 2 + 34) * 0.001f);
  }
  ...
  for (uint8_t i = 0; i < temperature_sensors; i++) {
    ESP_LOGI(TAG, "  Temperature Sensor %d: %.0f °C", i + 1, ((int16_t) ant_get_16bit(i * 2 + 62)) * 1.0f);
  }

  //  70   2  0x1C 0x00   Mosfet temperature          int16_t
  ESP_LOGI(TAG, "  Mosfet temperature: %.0f °C", ((int16_t) ant_get_16bit(70)) * 1.0f);

  //  72   2  0x1C 0x00   Balancer temperature        int16_t
  ESP_LOGI(TAG, "  Balancer temperature: %.0f °C", ((int16_t) ant_get_16bit(72)) * 1.0f);

  //  74   2  0x7E 0x16   Total voltage              uint16_t
  ESP_LOGI(TAG, "  Total voltage: %.2f V", ant_get_16bit(74) * 0.01f);

  //  76   2  0x00 0x00   Current                     int16_t
  ESP_LOGI(TAG, "  Current: %.1f A", ((int16_t) ant_get_16bit(76)) * 0.1f);

  //  78   2  0x60 0x00   State of charge            uint16_t
  ESP_LOGI(TAG, "  State of charge: %.0f %%", ((int16_t) ant_get_16bit(78)) * 1.0f);

  //  80   2  0x64 0x00   State of health            uint16_t
  ESP_LOGI(TAG, "  State of health: %.0f %%", ant_get_16bit(80) * 1.0f);

  //  82   1  0x01        Charge MOS status
  ESP_LOGI(TAG, "  Charge MOS status: %d", data[82]);

  //  83   1  0x02        Discharge MOS status
  ESP_LOGI(TAG, "  Disharge MOS status: %d", data[83]);

  //  84   1  0x00        Balancer status
  ESP_LOGI(TAG, "  Balancer status: %d", data[84]);

  //  85   1  0x00        Reserved
  //  86   4  0x80 0xC3 0xC9 0x01    Battery capacity            uint32_t
  ESP_LOGI(TAG, "  Battery capacity: %.2f Ah", ant_get_32bit(86) * 0.00001f);

  //  90   4  0x4F 0x55 0xB3 0x01    Battery capacity remaining  uint32_t
  ESP_LOGI(TAG, "  Battery capacity remaining: %.2f Ah", ant_get_32bit(90) * 0.00001f);

  //  94   4  0x08 0x53 0x00 0x00    Battery cycles capacity     uint32_t
  ESP_LOGI(TAG, "  Battery cycles capacity: %.2f Ah", ant_get_32bit(94) * 0.001f);

  //  98   4  0x00 0x00 0x00 0x00    Power?
  ESP_LOGI(TAG, "  Power: %.0f W", ((int32_t) ant_get_32bit(98)) * 1.0f);

  // 102   4  0x6B 0x28 0x12 0x00    Total runtime
  ESP_LOGI(TAG, "  Total runtime: %d s", ant_get_32bit(102));

  // 106   4  0x00 0x00 0x00 0x00    Balanced cell bitmask
  ESP_LOGI(TAG, "  Balanced cell bitmask: %d", ant_get_32bit(106));

  // 110   2  0x11 0x10              Maximum cell voltage
  ESP_LOGI(TAG, "  Maximum cell voltage: %.2f V", ant_get_16bit(110) * 0.001f);

  // 112   2  0x01 0x00              Maximum voltage cell
  ESP_LOGI(TAG, "  Maximum voltage cell: %d", ant_get_16bit(112));

  // 114   2  0x11 0x10              Minimum cell voltage
  ESP_LOGI(TAG, "  Minimum cell voltage: %.2f V", ant_get_16bit(114) * 0.001f);

  // 116   2  0x01 0x00              Minimum voltage cell
  ESP_LOGI(TAG, "  Minimum voltage cell: %d", ant_get_16bit(116));

  // 118   2  0x00 0x00              Voltage difference
  ESP_LOGI(TAG, "  Voltage difference: %.2f V", ant_get_16bit(118) * 0.001f);

  // 120   2  0x11 0x10              Average cell voltage
  ESP_LOGI(TAG, "  Average cell voltage: %.2f V", ant_get_16bit(120) * 0.001f);
  ...
  // 132   4  0x7D 0x2E 0x00 0x00    Accumulated discharging energy or capacity?
  ESP_LOGI(TAG, "  Accumulated discharging capacity: %d Ah?", ant_get_32bit(132));

  // 136   4  0x94 0x77 0x00 0x00    Accumulated charging energy or capacity?
  ESP_LOGI(TAG, "  Accumulated charging capacity: %d Ah", ant_get_32bit(136));

  // 140   4  0xDE 0x07 0x00 0x00    Accumulated discharging time
  ESP_LOGI(TAG, "  Accumulated discharging time: %d s", ant_get_32bit(140));

  // 144   4  0x77 0x76 0x00 0x00    Accumulated charging time
  ESP_LOGI(TAG, "  Accumulated charging time: %d s", ant_get_32bit(144));
...
void AntBms::update() {
  ...
  if (this->enable_fake_traffic_) {
    //    this->on_ant_bms_data_(
    //        FUNCTION_READ_ALL,
    //        {
    //            0xAA, 0x55, 0xAA, 0xFF, 0x01, 0x0B, 0x0D, 0x0C, 0x0D, 0x0C, 0x0D, 0x0B, 0x0D, 0x0A, 0x0D, 0x0C, 0x0D,
    //            0x0B, 0x0D, 0x0C, 0x0D, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    //            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    //            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    //            0x00, 0x00, 0xFF, 0xFF, 0xFF, 0x95, 0x29, 0x00, 0x00, 0x00, 0x00, 0x02, 0xE3, 0xDB, 0x95, 0x01, 0x26,
    //            0x22, 0x63, 0x01, 0xB1, 0x6E, 0xBA, 0x00, 0x0F, 0x00, 0x0F, 0x00, 0x0D, 0x00, 0x0D, 0x00, 0x00, 0x00,
    //            0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFE, 0xE3, 0x01, 0x0D, 0x0C, 0x04,
    //            0x0D, 0x0A, 0x0D, 0x0B, 0x08, 0x00, 0x00, 0x00, 0x7E, 0x00, 0x7A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    //            0x04, 0x0F, 0x0E, 0xDF,
    //        });

    this->on_ant_bms_data_(
        FUNCTION_READ_ALL,
        {});
  }
}
